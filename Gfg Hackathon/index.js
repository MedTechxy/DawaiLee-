...
app.post('/DLogin',(req,res)=>{
    var email=req.body.email;
    var Password=req.body.password;
    var sql=`select Email,password,Name from DoctorInfo where Email=? AND password=?`;
    con.query(sql,[email,Password],function(err,results){
     if(err) throw err;
     else{
        var em = results.map(result => result.Email);
        ...
        if(em==email && pass==Password){
            var username = results[0].Name; //get the name of the doctor from the query result
            res.render("DrDash", { username: username}); //pass the username to the EJS file
...
app.post('/AddP', function(req, res) {
        ...
        res.render('AddPatient', {name: "Name will display here", history: "History Will Display here"});
      });  
    ...
    con.query(sql, [pid], function(err, results) {
      if (err) throw err;
        ...
        if (results.length > 0) {
          var data = results[0];
          console.log(data);
          res.render('AddPatient', {name: data.Name, history: data.Conditions, date:today});
        } else {
          notifier.notify({
            title: 'Error!!!',
            message: 'Please check Patient Id'
          });
          res.render('AddPatient',{name: "", history:"",date:""});
          console.log('done');
        }
      }
    });
